...
export default function AnalysisContent({ showHeader = false }: { showHeader?: boolean }) {
  ...
  const [detailJob, setDetailJob] = useState<Job51Item | null>(null)
  ...
  const loadList = async (toPage = page, toSize = size) => {
    ...
    try {
      ...
      const data: PagedResult51 = await res.json()
      setItems(data.items || [])
  ...
  const exportCSV = async () => {
    try {
      ...
      a.href = url
      a.download = `job51_jobs_${new Date().toISOString().slice(0, 10)}.csv`
      a.click()
  ...
  return (
    <div className="w-full max-w-full space-y-3.5 overflow-hidden">
      {showHeader && (
        <PageHeader
          title="51job 投递分析"
          subtitle="基于 job51_data 表的统计图与列表分析"
      ...
      <div className="rounded-xl bg-white dark:bg-neutral-900 border border-slate-200/80 dark:border-neutral-800 shadow-xs overflow-hidden">
        <div className="px-3.5 py-2.5 border-b border-slate-100 dark:border-neutral-800 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs">
            ...
            <span className="font-semibold text-slate-900 dark:text-white">51job 岗位数据列表</span>
        ...
        <div className="w-full overflow-x-auto">
          <table className="w-full text-xs text-left">
            <thead className="bg-slate-50 dark:bg-neutral-800/60 text-slate-600 dark:text-neutral-400 font-semibold border-b border-slate-200 dark:border-neutral-800">
              <tr>
                <th className="px-4 py-3 min-w-[160px]">公司名称</th>
                <th className="px-4 py-3 min-w-[180px]">招聘岗位</th>
                <th className="px-4 py-3 min-w-[110px]">薪资与地点</th>
                <th className="px-4 py-3 min-w-[120px]">HR 与发布时间</th>
                <th className="px-4 py-3 min-w-[90px]">投递状态</th>
                <th className="px-4 py-3 text-right min-w-[100px]">操作</th>
              </tr>
            </thead>
                  ...
                  >
                    ...
                    <td className="px-4 py-3 align-top">
                      ...
                      <div className="text-[10px] text-slate-400 mt-0.5">
                        发布: {it.publishTime || formatDateOnly(it.createdAt) || "-"}
                      </div>
                    </td>
          ...
          <div className="w-full max-w-xl p-6 rounded-xl bg-white dark:bg-neutral-900 border border-slate-200/80 dark:border-neutral-800 shadow-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3 p-3 rounded-xl bg-slate-50 dark:bg-neutral-800/40">
                <div>
                  ...
                  <span className="font-semibold text-slate-700 dark:text-neutral-300">{detailJob.hrName || "未注明"}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px]">发布时间</span>
                  <span className="font-semibold text-slate-700 dark:text-neutral-300">{detailJob.publishTime || formatDateOnly(detailJob.createdAt) || "未知"}</span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 pt-3 border-t border-slate-100 dark:border-neutral-800 flex items-center justify-between">
              {detailJob.jobUrl ? (
                ...
                >
                  在 51job 平台打开原始岗位链接 <BiLinkExternal />
                </a>
              ) : <div />}
              <button
                onClick={() => setDetailJob(null)}
